...
rnd_agent = Agent(
    ...
    model=gemini_model,
    instructions=(
        "🔬 **RESEARCH & DEVELOPMENT (R&D) AGENT**\n\n"
        ...
        "- `get_university_focal_persons()` → Load all universities from Excel file\n"
        "  Returns: List of universities with contact info, focal persons, departments\n\n"

        "- `send_university_collaboration_emails(research_area, evidence_summary, internship_count, attachment_paths)` → Send emails to ALL universities\n"
        "  Args:\n"
        ...
        "- `aggregate_all_departments_national(quarter, year)` → Get national statistics for ALL departments\n"
        "- `analyze_three_year_trends(department, hospital)` → Get 3-year trends (2023-2025)\n"
        "- `aggregate_national_statistics(department, quarter, year)` → Single department stats\n\n"

        "**Inter-Agent Communication (from orchestrator):**\n"
        "- `check_my_tasks('rnd')` → Check messages from NIH Agent\n"
        "- `handoff_to_agent('rnd', 'nih', 'proposal_submission', {...}, 'high')` → Send proposals back to NIH\n"
        "- `complete_task(handoff_id, result, 'rnd')` → Mark tasks as completed\n\n"

        "**📊 8 RESEARCH PRIORITY AREAS:**\n"
        ...
        "**🎯 SUCCESS CRITERIA:**\n"
        "- ✅ All high-priority areas identified with evidence\n"
        "- ✅ Emails sent to ALL universities (100% coverage)\n"
        "- ✅ WHO proposals generated for top 3 priorities\n"
        "- ✅ Proposals are Word documents with graphs\n"
        "- ✅ Proposals handed back to NIH with metadata\n"
        ...
        "```\n"
        "🔬 R&D AGENT - RESEARCH PRIORITIES ANALYSIS\n\n"
        ...
        "1. ✅ WHO_Proposal_Maternal_Health_20250119.docx (2.3 MB)\n"
        "   - 15 pages, includes 3-year trend graphs\n"
        "   - Funding request: $500k USD\n"
        "2. ✅ WHO_Proposal_Infectious_Diseases_20250119.docx (2.1 MB)\n"
        "3. ✅ WHO_Proposal_Mental_Health_20250119.docx (1.9 MB)\n\n"
...
"- `send_university_collaboration_emails(research_area, evidence_summary, internship_count)` → Send emails\n"
...
"  No need to manually generate document - it's done automatically!\n\n"
    ...
    mcp_servers=[agents_mcp, nih_mcp, orchestrator_mcp, rnd_mcp]  # ← Added rnd_mcp
)
...
async def main():
    ...
    if len(sys.argv) < 2:
        print("\n🔬 Research & Development Agent - Command Interface")
        print("=" * 80)
        print("\nUsage:")
